...
type Combiner struct {
	...
	input struct {
		...
		// Previous accepted row, and the next row being considered.
		prev, next inputRowState
	}
...
// NewCombiner returns a Combiner over rows having the |input| Shape, and which
// emits rows & aggregates according to the Query Shape and applicable filters.
// It returns an error if the |input| Shape or |query| are not well-formed with
// respect to each other, and to the Schema.
func NewCombiner(schema Schema, input Shape, query QuerySpec) (*Combiner, error) {
	// Index dimension tags by their order within the input Shape.
	...
	for _, tag := range query.Dimensions {
		if ind, ok := dimIndex[tag]; !ok {
			return nil, errors.Errorf("Query Dimension tag %d not in input Shape (%v)", tag, input.Dimensions)
	...
	for _, f := range query.Filters {
		if ind, ok := dimIndex[f.Dimension]; !ok {
			return nil, errors.Errorf("Filter Dimension tag %d not in input Shape (%v)", f.Dimension, input.Dimensions)
	...
	// Index metric tags by their order within the Query Shape.
	var metInvIndex = make(map[MetTag]int)
	for m, tag := range query.Metrics {
	...
	if len(metInvIndex) != 0 {
		return nil, errors.Errorf("Query Metrics not in Input Shape (%v)", metInvIndex)
	}
	...
	cb.output.metrics = query.Metrics

	cb.Flush() // Initialize Aggregates.
...
func (cb *Combiner) Combine(key, value []byte) (flushed, seek bool, err error) {
	var firstRow = cb.input.prev.key == nil
	...
	for m, tag := range cb.input.metric.tags {
		if ind := cb.input.metric.reorder[m]; ind != -1 {
			...
			value, err = cb.Schema.DequeEncodedMetric(value, tag)
		}
		if err != nil {
			return
		}
...
// Flush the key and aggregates currently being grouped.
func (cb *Combiner) Flush() {
...
func (cb *Combiner) Value(b []byte) []byte {
	for m, tag := range cb.output.metrics {
		b = cb.Schema.EncodeMetric(b, tag, cb.output.flushed.aggs[m])
	}
	return b
}

// Metrics returns the last Flushed Aggregates.
...
func (cb *Combiner) transition(key []byte) (rejected bool, err error) {
	...
	for d := 0; d != len(cb.input.dimension.tags) && !rejected; d++ {
		var (
			pBeg, pEnd = cb.input.prev.offsets[d], cb.input.prev.offsets[d+1]
			pField     = cb.input.prev.key[pBeg:pEnd]
		...
		if cmp := bytes.Compare(pField, cb.input.next.key[nBeg:nEnd]); !restartRanges && cmp > 0 {
			log.WithFields(log.Fields{
				"pField":   pField,
				"prev.key": cb.input.prev.key,
				"nField":   cb.input.next.key[nBeg:],
				"next.key": cb.input.next.key,
			}).Panicf("key ordering invariant violated")
		} else if cmp == 0 && len(pField) != 0 {
			// Dimension didn't change from |prev| to |next|.
			cb.input.next.offsets = append(cb.input.next.offsets, nEnd)
			cb.input.next.updated = append(cb.input.next.updated, false)
			cb.input.next.rangeInd = append(cb.input.next.rangeInd, cb.input.prev.rangeInd[d])
			continue
		}
		...
		if rem, err := cb.DequeEncodedDimension(cb.input.next.key[nBeg:], cb.input.dimension.tags[d]); err != nil {
		...
		if !restartRanges {
			...
			rangeInd = cb.input.prev.rangeInd[d]
			restartRanges = true
		}
	...
	if !rejected {
		cb.input.next, cb.input.prev = cb.input.prev, cb.input.next // Transition to |next|.
	}
	return rejected, nil
}
